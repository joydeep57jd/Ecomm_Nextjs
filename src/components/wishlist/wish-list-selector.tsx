...
function WishlistSelector({ handleCloseModal, product }: Props) {
    ...
    const save = async () => {
        ...
        try {
            ...
            await saveWishlistItem({
                ...
                wishListCategoryName: wishListCategory?.find(collection => collection.wishListCategoryId == collectionId)?.wishListCategoryName || ''
            })
    ...
    return isAddingNewCollection ? <SaveCollection handleCloseModal={closeSaveCollection} /> : <Dialog fullWidth open onClose={() => handleCloseModal(false)}>
        <DialogTitle>
        ...
        <DialogContent>
            <FormControl fullWidth sx={{ mb: 2 }}>
                ...
                <Select labelId="demo-simple-select-label" id="demo-simple-select" label="Collections" value={collectionId}
                    error={canShowRequiredError} onChange={(e) => onChange(e.target.value as number)}>
                    {
                        wishListCategory?.map(collection => <MenuItem key={collection.wishListCategoryId}
                            value={collection.wishListCategoryId}>{collection.wishListCategoryName}</MenuItem>
                        )
                    }
                    {
                        !wishListCategory?.length && <MenuItem
                            value={''}>{wishListCategory ? 'No collection added yet' : 'Please wait...'}</MenuItem>
                    }
                </Select>
                {
                    ...
                    <Typography variant='inherit' sx={{
                        ...
                        marginLeft: '14px',
                        mt: '4px'
                    }}>Please select a collection</Typography>
                }
            </FormControl>
