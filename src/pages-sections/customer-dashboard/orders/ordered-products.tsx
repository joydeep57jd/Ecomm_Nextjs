...
export default function OrderedProducts({ order }: Props) {
  ...
  return (
    <>
          ...
          <FlexBetween px={2} py={1} flexWrap="wrap" key={ind} gap={1}>
            <FlexBox gap={2} alignItems="center">
              <Avatar variant="rounded" sx={{ height: 60, width: 60, backgroundColor: "grey.50" }}>
                <Image alt={item.imageAlt} src={item.imageName} width={50} height={50} style={{
                  height: 60,
                  width: 60,
                  borderRadius: '100%',
                  objectFit: 'contain',
            ...
            Product properties: {item.itemVariantId}
          </Typography> */}
      ...
      {
        !!selectedProduct &&
        <OrderItemRating handleCloseModal={handleCloseModal} itemId={selectedProduct.itemId} variantId={selectedProduct.itemVariantId} product={selectedProduct} />
      }
    </>
  )
}
