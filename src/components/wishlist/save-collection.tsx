...
function SaveCollection({ handleCloseModal, wishListCategory }: Props) {
    ...
    return <Dialog fullWidth open onClose={() => handleCloseModal()}>
        <DialogTitle>
        ...
        <Divider />
        <DialogContent>
            <TextField error={canShowRequiredError} helperText={canShowRequiredError && 'Please enter collection name'} fullWidth id="outlined-basic" value={collectionName} onChange={onNameChange} label="Collection Name" variant="outlined" sx={{ mb: 2 }} />
        </DialogContent>
        <Divider />
