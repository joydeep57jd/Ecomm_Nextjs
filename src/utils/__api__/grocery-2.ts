...
const getNavigationList = cache(async () => {
  const response = await axios.get<CategoryNavItem[]>("/api/grocery-2/category-navigation")
  return response.data
})
...
const getTestimonials = cache(async () => {
  const response = await axios.get<Testimonial[]>("/api/grocery-2/testimonial-list")
  return response.data
})
